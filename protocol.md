...
If either side does not send an INFO packet within 5 seconds, the connection may still proceed, but version 1 of the protocol is used. If there are any other critical compatibility issues detected by either party during the handshake, the connection should be immediately closed with a CLOSE packet with reason `0x04` and a stream ID of 0. A connection can still proceed if there are conflicts, provided both parties have the error handling capabilities to dispose of any packets defined by an incompatible extension.
